...
import { EditSupplier } from 'src/sections/companies/edit-supplier';
...
export const CompanySearch = ({ data, clients }) => {
  ...
  const handleSearch = () => {
    ...
    for (let i = 0; i < data.length; i++) {
      let supplier = data[i].name
      let supplierString = supplier.replace(/\s/g, '').toUpperCase()
      let searchValue = value.replace(/\s/g, '').toUpperCase()
      if (supplierString.includes(searchValue) || calculateLevenshteinDistance(supplier, value.toUpperCase())<=2) {
  ...
  return (
    <Card sx={{ p: 2 }}>
      ...
      }
      {
        ...
        <EditSupplier onCancel={() => setAddClient(false)} data={clientResult[expanded]} suppliers={data} clients={clients}/>
      ) : null
      }
    </Card>
