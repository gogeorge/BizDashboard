...
export const CustomersSearch = ({ data, projects }) => {
  ...
  const getProject = (val) => {
    const filteredJsonObjects = projects.filter(obj => obj.client.toUpperCase() === val.name.toUpperCase());
  ...
  const handleSearch = () => {
    ...
    for (let i = 0; i < data.length; i++) {
      let country = data[i]
      for (let j = 0; j < country.folders.length; j++) {
        let client = data[i].folders[j].name

        let clientString = client.replace(/\s/g, '').toUpperCase()
        let searchValue = value.replace(/\s/g, '').toUpperCase()
        if (clientString.includes(searchValue) || calculateLevenshteinDistance(client, value.toUpperCase())<=2) {
          cli.push(data[i].folders[j])
          fol.push(data[i].folders[j])
          fil.push(data[i].folders[j])
          setResult(true)
        }
      }
    }
          ...
          {console.log('pre: ', clientResult.map(c => console.log(c)))}
       {
          ...
          <CardContent>
            <Typography variant="h5">Folders</Typography>
            {
              ...
              {/* <Card 
                key={folders[i].folderId}
                onClick={(e) => {
                  window.open('https://drive.google.com/drive/folders/' + folders[i].folderId)
                  setDBWarning(true)
                }}
                >
                  {console.log(i, folders[i])}
                <CardContent>
                  <Typography variant="h6">{folders[i].folders[].name}</Typography>
                  <Typography variant="body1">{folders[i].folderId}</Typography>
                </CardContent>
              </Card> */}
          </CardContent>
          <CardContent>
          ...
          <Grid container spacing={2} style={{ width: '135%'}}>
              ...
              <Paper elevation={3} style={{ height: '100%', padding: '20px', textAlign: 'center' }}>
                ...
                openDel ? (
                      ...
                      >
                        ...
                        <DialogContent>
                          <DialogContentText id="alert-dialog-description">
                            Are you sure you want to delete the {clientResult[expanded].name} client? This action cannot be undone.
                          </DialogContentText>
                        </DialogContent>
                        <DialogActions>
            ...
            <Typography variant="h5">Folders</Typography>
            {
              folders[0].folders.length > 0 ? (
                <div>
                  <Card 
                    key={folders[0].folders[0].folderId}
                    onClick={(e) => {
                      window.open('https://drive.google.com/drive/folders/' + folders[0].folders[0].folderId)
                      setDBWarning(true)
                    }}
                    >
                    <CardContent>
                      <Typography variant="h6">{folders[0].folders[0].name}</Typography>
                      <Typography variant="body1">{folders[0].folders[0].folderId}</Typography>
                    </CardContent>
                  </Card>
                </div>
        ...
        <EditClient onCancel={() => setAddClient(false)} data={clientResult[expanded]} clients={data}/>
      ) : null
      }
    </Card>
